...
class WeatherAPIGetter
  ...
  def self.get_weather_data # gets info from API
    ...
    country_code = self.gimme_cc
    #---
    Query.create(city: city, country_code: country_code)

    apikey = "304bcd251ded0521b8be10e3c5743f37"
    weather_json = RestClient.get("api.openweathermap.org/data/2.5/forecast?q=#{city},#{country_code}&APPID=#{apikey}&units=imperial")
